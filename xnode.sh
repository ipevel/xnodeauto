...
set_alias() {
    ...
    save_aliases
    echo "${ICON_OK} 已设置节点 $node_id 别名为: $alias"
}
...
remove_alias() {
    ...
    if [[ -n "${node_aliases[$node_id]}" ]]; then
        ...
        save_aliases
        echo "${ICON_OK} 已删除节点 $node_id 的别名"
    fi
}
...
status() {
    ...
    if [[ -z "$nodes" ]]; then
        echo "[WARN] 没有运行任何节点"
        echo ""
        echo "[INFO] 使用 xnode add-node <节点ID> 添加节点"
    else
        echo "  节点ID  别名              状态      "
        echo "------  ----------------  ------"
...
restart_all() {
    ...
    if [[ -z "$nodes" ]]; then
        ...
        while IFS= read -r line; do
            ...
            echo "[->] 重启节点 $node_id ($alias)..."
            systemctl restart "$unit" 2>/dev/null

            if systemctl is-active "$unit" > /dev/null 2>&1; then
                echo "[OK] 节点 $node_id 已重启"
            else
                echo "[ERR] 节点 $node_id 重启失败"
            fi
        done <<< "$nodes"
    fi
...
add_node() {
    ...
    if [[ -z "$node_id" ]]; then
        echo "${ICON_ERR} 用法: xnode add-node <节点ID> [别名]"
        return 1
    fi
    
    # 验证节点ID必须是数字
    if ! [[ "$node_id" =~ ^[0-9]+$ ]]; then
        echo "${ICON_ERR} 节点ID必须是数字"
        return 1
    fi

    echo "============================================"
    echo "[GEAR] 添加节点 $node_id"
    echo "============================================"
    echo ""
    ...
    if [[ -f /etc/xboard-node/sync.yml ]]; then
        ...
        echo "${ICON_OK} 已添加节点 $node_id 到配置"
    else
        echo "${ICON_ERR} 配置文件不存在,请先安装"
        return 1
    fi
...
remove_node() {
    ...
    if [[ -z "$node_id" ]]; then
        echo "${ICON_ERR} 用法: xnode remove-node <节点ID>"
        return 1
    fi

    echo "============================================"
    echo "[GEAR] 删除节点 $node_id"
    echo "============================================"
    echo ""
...
set_node_alias() {
    ...
    if [[ -z "$node_id" || -z "$alias" ]]; then
        echo "${ICON_ERR} 用法: xnode set-alias <节点ID> <别名>"
        return 1
    fi
...
show_version() {
    ...
    if [[ -f /usr/local/bin/xboard-node ]]; then
        ...
        if [[ -z "$extracted_xb_ver" ]]; then
            ...
            current_xb_ver="${extracted_xb_ver}"
        fi
        echo "  xboard-node      ${current_xb_ver}"
    else
        echo "  xboard-node      未安装"
    fi
    ...
    if [[ -f /usr/local/bin/sync-nodes ]]; then
        ...
        if [[ -z "$extracted_sync_ver" ]]; then
            ...
            current_sync_ver="${extracted_sync_ver}"
        fi
        echo "  sync-nodes       ${current_sync_ver}"
    else
        echo "  sync-nodes       未安装"
    fi
...
show_node_management_menu() {
    while true; do
        ...
        echo "运行中: ${RUNNING_NODES} / 总数: ${TOTAL_NODES}"
        echo ""

        # 显示节点列表
        if [[ -f /etc/xboard-node/sync.yml ]]; then
            load_aliases
            local config_nodes=$(ls /etc/xboard-node/*.yml 2>/dev/null | grep -v sync.yml | grep -v node_alias.yml)
            if [[ -n "$config_nodes" ]]; then
                echo "节点ID  别名              状态"
                echo "------  ----------------  ------"
                for config_file in $config_nodes; do
...
update_all() {
    ...
    for file in /etc/xboard-node/*.yml /etc/xboard-node/*.yaml; do
        [ -f "$file" ] && cp "$file" "$BACKUP_DIR/" && echo "  [OK] $(basename $file)"
    done
    [ -f "/etc/xboard-node/node_alias.yml" ] && cp /etc/xboard-node/node_alias.yml "$BACKUP_DIR/" && echo "  [OK] node_alias.yml"
    echo "[OK] 配置已备份到: $BACKUP_DIR"
    echo ""
    ...
    for file in "$BACKUP_DIR"/*.yml "$BACKUP_DIR"/*.yaml; do
        [ -f "$file" ] && cp "$file" /etc/xboard-node/ && echo "  [OK] $(basename $file)"
    done
    [ -f "$BACKUP_DIR/node_alias.yml" ] && cp "$BACKUP_DIR/node_alias.yml" /etc/xboard-node/ && echo "  [OK] node_alias.yml"
    rm -rf "$BACKUP_DIR"
    echo "[OK] 配置恢复完成"
    echo ""
